...
/*	MEGA MENU
/*-----------------------------------------------------------------------------------*/
function ebor_register_services() {

    $labels = array(
        'name' => __( 'Services', 'reflex' ),
        'singular_name' => __( 'Services Item', 'reflex' ),
        'add_new' => __( 'Add New', 'reflex' ),
        'add_new_item' => __( 'Add New Services Item', 'reflex' ),
        'edit_item' => __( 'Edit Services Item', 'reflex' ),
        'new_item' => __( 'New Services Item', 'reflex' ),
        'view_item' => __( 'View Services Item', 'reflex' ),
        'search_items' => __( 'Search Services Items', 'reflex' ),
        'not_found' => __( 'No Services Items found', 'reflex' ),
        'not_found_in_trash' => __( 'No Services Items found in Trash', 'reflex' ),
        'parent_item_colon' => __( 'Parent Services Item:', 'reflex' ),
        'menu_name' => __( 'Services', 'reflex' ),
    );
    ...
    $args = array(
        ...
        'description' => __('Mega Menus entries for Slowave.', 'reflex'),
...
add_action( 'init', 'ebor_register_services' );

if(!( function_exists('ebor_reflex_page_header') )){
	function ebor_reflex_page_header( $title = false, $subtitle = false, $image = false, $show_cats = false ){
		...
		} elseif( get_post_meta($post->ID, '_ebor_use_filters', 1) == 'on' || $show_cats == true ){

			$cats = get_categories('taxonomy=portfolio-category');
			echo '<ul class="works-filter clearfix"><li><a id="all" href="#" data-filter="*" class="active"><span>'. __('All','reflex') .'</span></a></li>';
				foreach ($cats as $cat){
					echo '<li><a href="#" data-filter=".' . $cat->slug . '"><span>' . $cat->name . '</span></a></li>';
				}
			echo '</ul>';

		} elseif( $subtitle ){

			echo '<p class="white">'. htmlspecialchars_decode($subtitle) .'</p>';

		}

		if( is_singular('portfolio') || is_singular('post') ) {

			next_post_link('%link', '<img src="'. trailingslashit( get_template_directory_uri() ) .'images/bxslider/prev.png" alt="next post" />' );
			previous_post_link('%link', '<img src="'. trailingslashit( get_template_directory_uri() ) .'images/bxslider/next.png" alt="prev post" />' );
...
function pw_gallery_field( $field, $meta ) {
	...
	echo '	<input type="button" class="button button-primary manage" value="' . ( ! empty( $field['button'] ) ? $field['button'] : 'Manage gallery' ) . '" />';
	echo '	<input type="button" class="button ebor-gallery-remove" value="' . ( ! empty( $field['button'] ) ? $field['button'] : 'Delete All Gallery Items' ) . '" />';
	echo '</div>';
...
function pw_gallery_field_sanitise( $meta_value, $field ) {
	( empty( $meta_value ) ) ? $meta_value = '' : $meta_value = explode( ',', $meta_value );
	return $meta_value;
}
...
if(!( function_exists('tcb_add_post_thumbnail_column') )){
	function tcb_add_post_thumbnail_column($cols){
	  $cols['tcb_post_thumb'] = __('Featured Image','reflex');
	  return $cols;
	}
}
...
if(!( function_exists('ebor_pagination') )){
	function ebor_pagination($pages = '', $range = 2){
		...
		if(1 != $pages){
			$output .= "<div class='silver-bg container inner-pad text-center'><h3 class='main-heading black'>" . __('More Posts', 'reflex') . "</h3>";
			if($paged > 2 && $paged > $range+1 && $showitems < $pages) $output .= "<a class='btn btn-reflex btn-reflex-dark' href='".get_pagenum_link(1)."'>".__('First','reflex')."</a> ";

			for ($i=1; $i <= $pages; $i++){
				if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems )){
					$output .= ($paged == $i)? "<a class='active btn btn-reflex btn-reflex-dark' href='".get_pagenum_link($i)."'>".$i."</a> ":"<a class='btn btn-reflex btn-reflex-dark' href='".get_pagenum_link($i)."'>".$i."</a> ";
				}
			}

			if ($paged < $pages-1 &&  $paged+$range-1 < $pages && $showitems < $pages) $output .= "<a class='btn btn-reflex btn-reflex-dark' href='".get_pagenum_link($pages)."'>".__('Last','reflex')."</a> ";
			$output.= "</div>";
		}
		  ...
		  <div class="message">
		    <div class="arrow-box">
		      <div class="comment-info">
		        <?php printf('<h3 class="sub-heading">%s</h3>', get_comment_author_link()); ?>
		        <div class="meta">
		      ...
		      <p><em><?php _e('Your comment is awaiting moderation.', 'reflex') ?></em></p>
		      <?php endif; ?>
		    </div>
		  </div>
		</li>
